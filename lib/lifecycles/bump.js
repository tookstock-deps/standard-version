...
async function Bump (args, version) {
  ...
  if (args.useVersion) {
    newVersion = args.useVersion
    updateConfigs(args, newVersion)
  } else if (!args.firstRelease) {
...
function updateConfigs (args, newVersion) {
  ...
  args.bumpFiles.forEach(function (bumpFile) {
    ...
    if (! updater) {
      console.warn("No updater for file", bumpFile)
      return
    }
    const configPath = path.resolve(process.cwd(), updater.filename)
    try {
      if (dotgit.ignore(configPath)) { console.warn("Updating .gitignore'd file", configPath) }
